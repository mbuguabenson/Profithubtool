...
const BulkTraderTab = observer(() => {
    ...
    const [newStrategy, setNewStrategy] = React.useState<Omit<TStrategy, 'id' | 'status' | 'total_profit' | 'total_trades' | 'last_result_time' | 'is_active'>>({
        symbol: 'R_100',
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'RUNNING': return 'var(--text-profit-success)';
            case 'BUYING': return 'var(--text-loss-danger)'; // Or orange?
            case 'WON': return 'var(--text-profit-success)';
            case 'LOST': return 'var(--text-loss-danger)';
            case 'ERROR': return 'red';
            case 'IDLE': return 'var(--text-general)';
            default: return 'var(--text-general)';
        }
    };
    ...
    return (
        <div style={{ padding: '20px', color: 'var(--text-general)', height: '100%', overflowY: 'auto' }}>
            ...
            <p>
                Run multiple strategies simultaneously. Trades are executed directly on tick reception for maximum speed.
                <br />
                <span style={{ color: 'var(--text-loss-danger)' }}>Warning: High frequency trading can hit API rate limits.</span>
            </p>
            ...
            <div style={{ 
                ...
                <div>
                    ...
                    >
                        {active_symbols.map(s => <option key={s.symbol} value={s.symbol}>{s.display_name}</option>)}
                    </select>
                </div>
                <div>
                    <label>Type</label><br/>
                ...
                <div>
                    <label>Parameter (Digit/Barrier)</label><br/>
                    <input 
                        type="number" 
                        value={newStrategy.parameter} 
                        onChange={(e) => setNewStrategy({...newStrategy, parameter: Number(e.target.value)})}
                        style={{ background: 'var(--general-main-1)', color: 'var(--text-general)', border: '1px solid var(--border-normal)', padding: '5px', width: '60px' }}
                    />
                </div>
                 <div>
                    ...
                    <input 
                        ...
                        style={{ background: 'var(--general-main-1)', color: 'var(--text-general)', border: '1px solid var(--border-normal)', padding: '5px', width: '80px' }}
                    />
                </div>
                <div>
                    <button 
                ...
                {strategies.map(strategy => (
                    <div key={strategy.id} style={{ 
                        ...
                        <div style={{ fontSize: '0.9em', marginBottom: '10px' }}>
                            {strategy.contract_type} {['DIGITMATCH', 'DIGITDIFF', 'DIGITOVER', 'DIGITUNDER'].includes(strategy.contract_type) && `(${strategy.parameter})`}
                            <br/>
                            Stake: {strategy.amount} {strategy.currency}
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                                <div style={{ color: strategy.total_profit >= 0 ? 'var(--text-profit-success)' : 'var(--text-loss-danger)' }}>
                            ...
                            <div style={{ display: 'flex', gap: '5px' }}>
                                <button 
                                    ...
                                    style={{ 
                                        background: strategy.is_active ? 'var(--text-loss-danger)' : 'var(--text-profit-success)', 
                                        color: 'white', 
                                        border: 'none', 
                                        padding: '5px 10px', 
                                        borderRadius: '4px',
